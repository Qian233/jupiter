...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // encode相当于把rick sun 转换成rick+sun了, 符合标准
		...
		ResponseHandler<List<Item>> responseHandler = new ResponseHandler<List<Item>>() {
			...
			public List<Item> handleResponse(final HttpResponse response) throws ClientProtocolException, IOException {
				...
				String respondBody = EntityUtils.toString(entity); // 一个json 格式的string
				JSONArray array = new JSONArray(respondBody);
